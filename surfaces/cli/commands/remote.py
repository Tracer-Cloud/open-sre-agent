...
from surfaces.cli.commands.remote_health import _save_remote_base_url, run_remote_health_check
...
def _resolve_remote_ops_scope(ctx: click.Context) -> tuple[RemoteOpsProvider, RemoteServiceScope]:
    ...
    from surfaces.cli.wizard.store import load_remote_ops_config
...
def _persist_remote_ops_scope(scope: RemoteServiceScope) -> None:
    from surfaces.cli.wizard.store import save_remote_ops_config
...
def _run_remote_interactive(ctx: click.Context) -> None:
    ...
    from surfaces.cli.wizard.store import (
