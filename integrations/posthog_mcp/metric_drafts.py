...
the PostHog dialect (HogQL) and how to tell a real signup event from a login
stand-in in gather observations.
"""
...
def posthog_signup_cohort_resolved(evidence: Any, reply: str) -> bool:
    ...
    if posthog_reply_reports_cohort_unverified(reply):
        return False
...
def register_posthog_mcp_metric_drafts() -> None:
    """Opt PostHog MCP into the unformed-metric draft floor + cohort identity."""
    register_metric_query_draft(
        "posthog_mcp",
