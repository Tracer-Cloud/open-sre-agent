"""Mattermost credential resolution for alarm dispatch (watchdog, Hermes).

``server_url``/``auth_token``/``webhook_url`` reuse the scheduler's resolver
(:func:`platform.scheduler.credentials.resolve_mattermost_credentials`) so
task-params > integration-store > environment precedence stays identical
...
``MATTERMOST_DEFAULT_CHANNEL`` env) and raises a setup-friendly error when
nothing usable is configured.

Routing rule (matches ``mattermost_send_message``, the cron provider, and the
...
class MattermostCredentials:
    """Resolved Mattermost delivery target — token mode, webhook mode, or both.
...
def _store_default_channel() -> str:
    try:
        ...
        entry = resolve_effective_integrations().get("mattermost", {})
        ...
        logger.debug(
            "Failed to resolve Mattermost default_channel from the store: %s",
            exc,
            exc_info=True,
        )
        return ""
...
def _resolve_channel(channel_override: str | None) -> str:
    ...
    return os.getenv("MATTERMOST_DEFAULT_CHANNEL", "").strip()
...
def load_credentials_from_env(
    *,
    ...
    """Resolve Mattermost credentials for alarm dispatch.
    ...
    """
    from platform.scheduler.credentials import resolve_mattermost_credentials
    ...
    if has_pat:
        ...
        raise OpenSREError(
            "Mattermost has no channel to deliver to.",
            suggestion=(
                "Pass --chat-id, set a default channel during "
                "`opensre integrations setup mattermost`, or export "
                "MATTERMOST_DEFAULT_CHANNEL=<channel_id>."
            ),
        )
    ...
    if webhook_url:
        ...
        if stripped_override:
            raise OpenSREError(
                "An explicit channel needs Mattermost token credentials.",
                suggestion=(
                    "Configure server_url/auth_token with "
                    "`opensre integrations setup mattermost`, or drop --chat-id "
                    "to use the incoming webhook's fixed destination."
                ),
            )
        return MattermostCredentials(webhook_url=webhook_url)

    raise OpenSREError(
        "Mattermost is not configured.",
        suggestion=(
            "Configure Mattermost with `opensre integrations setup mattermost` "
            "(or `opensre onboard`), or export MATTERMOST_SERVER_URL, "
            "MATTERMOST_AUTH_TOKEN (or MATTERMOST_WEBHOOK_URL)."
        ),
    )
