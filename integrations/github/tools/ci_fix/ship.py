...
from integrations.github.tools.ci_fix.context import CI_TARGET_BRANCH, CiFixContext
...
def push_ci_fix(
    ...
    *,
    ...
    """Commit files changed by the fix run and push the repair branch."""
    ...
    try:
        ...
        if not changed:
            raise GitHubCiFixError(
                ERR_NO_CHANGES,
                f"CI fix for {_target_label(ctx)} produced no file changes; no push was made.",
                branch_name=ctx.head_branch,
            )
...
def _commit_message(ctx: CiFixContext, summary: str) -> str:
    ...
    if ctx.target_kind == CI_TARGET_BRANCH:
        target = ctx.target_branch or ctx.base_branch
