...
def test_bindable_output_forwards_paint_quiet_stdout() -> None:
    from platform.turn_host.bindable_output import BindableOutput
    ...
    with_paint = BindableOutput()
    with_paint.bind(_Inner())  # type: ignore[arg-type]
    assert with_paint.paint_quiet_stdout() is True

    without = BindableOutput()
    without.bind(_Plain())  # type: ignore[arg-type]
    assert without.paint_quiet_stdout() is False
