...
from core.agent_harness.session.memory_extraction import (
...
class TestExtraction:
    def test_valid_json_saves_memories(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        extract_memories_from_session(_FakeSession())
    ...
    def test_cap_of_five_memories(self, monkeypatch: pytest.MonkeyPatch) -> None:
        items = [_valid_item(f"mem-{i}") for i in range(MAX_MEMORIES_PER_SESSION + 3)]
        _patch_llm(monkeypatch, json.dumps(items))
        extract_memories_from_session(_FakeSession())
        assert len(list_memories()) == MAX_MEMORIES_PER_SESSION
...
class TestSkipConditions:
    def test_skips_short_sessions(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        session = _FakeSession(cli_agent_messages=[("user", "hi")] * (MIN_CHAT_MESSAGES - 1))
