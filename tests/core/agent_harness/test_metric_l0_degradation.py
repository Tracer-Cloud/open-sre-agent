...
def test_metric_ask_with_source_connected_still_answers() -> None:
    session = Session()
    ...
    def _answer(text: str, *_a: object, **_k: Any) -> Any:
        answer_calls.append(text)
        return type("Run", (), {"response_text": "ok"})()
...
def test_connected_source_without_a_live_query_still_drafts_hogql_and_setup() -> None:
    """No live query on the chat path — draft + setup, then stop."""
    session = Session()
    session.resolved_integrations_cache = {"posthog_mcp": {"configured": True}}
...
def test_without_evidence_kind_handoff_does_not_add_metric_cta() -> None:
    """User prose alone must not trigger L0 — action must tag the kind."""
    session = Session()
    session.resolved_integrations_cache = {}
    ...
    assert "/integrations setup" not in (result.assistant_response_text or "")
