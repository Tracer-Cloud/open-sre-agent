...
from app.agent.nodes.publish_findings.gitlab_writeback import (
    _build_mr_note,
...
def test_no_op_when_env_flag_off(state_with_gitlab):
    with (
        ...
        patch("app.agent.nodes.publish_findings.gitlab_writeback.post_gitlab_mr_note") as mock_post,
    ):
...
def test_failure_does_not_propagate(state_with_gitlab):
    with (
        ...
        patch("app.agent.nodes.publish_findings.gitlab_writeback.logger") as mock_logger,
    ):
        post_gitlab_mr_writeback(state_with_gitlab, "report")
