...
from platform_services.scheduler.runner import (
    _compute_fire_time,
    _make_trigger,
    _on_job_submitted,
    _pending_fire_times,
...
class TestRunTaskNow:
    def test_nonexistent_task(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(
            "platform_services.scheduler.runner.get_task",
    ...
    def test_runs_existing_task(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("platform_services.scheduler.runner.get_task", lambda _task_id: task)

        with patch("platform_services.scheduler.runner.execute_task") as mock_exec:
