...
from tools.grafana_annotations_tool import (
    _iso_to_epoch_ms,
...
def test_run_no_client() -> None:
    ...
    with patch("tools.grafana_annotations_tool._resolve_grafana_client", return_value=mock_client):
...
def test_run_forwards_basic_auth_to_client() -> None:
    ...
    mock_client.query_annotations.return_value = []
    with patch(
        "tools.grafana_annotations_tool._resolve_grafana_client", return_value=mock_client
    ) as resolve:
        query_grafana_annotations(
            grafana_endpoint="http://grafana",
