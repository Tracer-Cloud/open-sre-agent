...
def test_run_happy_path() -> None:
    ...
    with patch(
        "tools.eks_node_health_tool.build_k8s_clients", return_value=(mock_core_v1, MagicMock())
    ):
...
def test_run_handles_exception() -> None:
    with patch("tools.eks_node_health_tool.build_k8s_clients", side_effect=Exception("auth error")):
        result = get_eks_node_health(cluster_name="c1", role_arn="arn:aws:iam::123:role/r")
