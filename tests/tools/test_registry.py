...
def test_vendor_tool_discovery_is_explicit_legacy_allowlist() -> None:
    """Vendor modules are no longer an open-ended tool-discovery surface."""
    vendor_module_names = registry_module._iter_tool_module_names(registry_module.vendors_package)

    assert vendor_module_names == sorted(vendor_module_names)
    assert set(vendor_module_names) == set(registry_module._LEGACY_VENDOR_TOOL_MODULE_NAMES)


def test_new_vendor_modules_are_not_auto_discovered(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A new ``vendors.<name>`` package must not become agent-callable implicitly."""
    legacy_module: Any = ModuleType("vendors.allowed_legacy_vendor")

    @tool(
        name="legacy_vendor_tool",
        description="Allowed legacy vendor surface.",
        source="knowledge",
    )
    def legacy_vendor_tool() -> dict[str, str]:
        return {"status": "legacy"}

    legacy_vendor_tool.__module__ = legacy_module.__name__
    legacy_module.legacy_vendor_tool = legacy_vendor_tool

    unexpected_module: Any = ModuleType("vendors.unexpected_vendor")

    @tool(
        name="unexpected_vendor_tool",
        description="Unexpected vendor surface.",
        source="knowledge",
    )
    def unexpected_vendor_tool() -> dict[str, str]:
        return {"status": "unexpected"}

    unexpected_vendor_tool.__module__ = unexpected_module.__name__
    unexpected_module.unexpected_vendor_tool = unexpected_vendor_tool

    imported_vendor_modules: list[str] = []

    def fake_import(package: ModuleType, name: str) -> ModuleType:
        if package is registry_module.vendors_package:
            imported_vendor_modules.append(name)
            if name == "allowed_legacy_vendor":
                return legacy_module
            if name == "unexpected_vendor":
                return unexpected_module
        raise AssertionError(f"unexpected registry import: {package.__name__}.{name}")

    saved_external_packages = list(registry_module._external_tool_packages)
    registry_module._external_tool_packages.clear()
    try:
        monkeypatch.setattr(
            registry_module, "_LEGACY_VENDOR_TOOL_MODULE_NAMES", ("allowed_legacy_vendor",)
        )
        monkeypatch.setattr(registry_module.pkgutil, "iter_modules", lambda _path: [])
    ...
    assert [tool_def.name for tool_def in tools] == ["legacy_vendor_tool"]
    assert imported_vendor_modules == ["allowed_legacy_vendor"]
