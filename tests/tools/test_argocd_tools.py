...
from tools.argocd_application_diff_tool import ArgoCDApplicationDiffTool
from tools.argocd_application_status_tool import ArgoCDApplicationStatusTool
...
def test_argocd_status_tool_extracts_params_and_returns_application(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    monkeypatch.setattr(
        "tools.argocd_application_status_tool.make_argocd_client",
...
def test_argocd_diff_tool_reports_drift(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    monkeypatch.setattr(
        "tools.argocd_application_diff_tool.make_argocd_client",
        lambda *_args, **_kwargs: _FakeArgoCDClient(),
    )
