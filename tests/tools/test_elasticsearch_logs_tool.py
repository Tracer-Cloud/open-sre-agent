...
from tools.elasticsearch_logs_tool import ElasticsearchLogsTool
...
def test_run_returns_unavailable_when_no_client() -> None:
    tool = ElasticsearchLogsTool()
    with patch("tools.elasticsearch_logs_tool.make_client", return_value=None):
...
def test_run_happy_path() -> None:
    ...
    with patch("tools.elasticsearch_logs_tool.make_client", return_value=mock_client):
...
def test_run_forwards_basic_auth_credentials_to_make_client() -> None:
    ...
    with patch(
        "tools.elasticsearch_logs_tool.make_client", return_value=mock_client
    ) as mock_make_client:
        tool.run(
            query="*",
            url="https://my-cluster.example.com",
...
def test_make_client_forwards_basic_auth_to_elasticsearch_config() -> None:
    ...
    from tools.elasticsearch_logs_tool._client import make_client

    with (
        patch("tools.elasticsearch_logs_tool._client.ElasticsearchClient") as mock_client_cls,
        patch("tools.elasticsearch_logs_tool._client.ElasticsearchConfig") as mock_config_cls,
    ):
        make_client(
            url="https://my-cluster.example.com",
            username="admin",
