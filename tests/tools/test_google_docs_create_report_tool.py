...
def test_run_returns_failure_when_client_not_configured() -> None:
    ...
    with patch("tools.google_docs_create_report_tool.GoogleDocsClient", return_value=mock_client):
...
def test_run_handles_exception() -> None:
    with patch(
        "tools.google_docs_create_report_tool.GoogleDocsClient", side_effect=Exception("Auth error")
    ):
        result = create_google_docs_incident_report(
            title="Report",
            summary="Summary",
