...
def test_incident_io_tool_run_list(tool, monkeypatch):
    ...
    mock_client.list_incidents.return_value = {"success": True, "incidents": [{"id": "1"}], "total": 1}
    
    mock_make_client = MagicMock(return_value=mock_client)
    monkeypatch.setattr("app.tools.IncidentIoIncidentsTool.make_incident_io_client", mock_make_client)

    result = tool.run(api_key="test-key", action="list", status="open")
    
    mock_make_client.assert_called_once_with("test-key")
    mock_client.list_incidents.assert_called_once_with(status="open")
...
def test_incident_io_tool_run_add_timeline(tool, monkeypatch):
    ...
    mock_client.add_timeline_event.assert_called_once_with("inc-123", title="Test Update", description="This is a test.")
    assert result["success"] is True
