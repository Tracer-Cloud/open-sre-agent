...
from tools.prefect_worker_health_tool import PrefectWorkerHealthTool
...
def test_run_returns_unavailable_on_api_failure(tool: PrefectWorkerHealthTool) -> None:
    ...
    with patch("tools.prefect_worker_health_tool.make_prefect_client", return_value=mock_client):
...
def test_run_returns_unavailable_when_client_none(tool: PrefectWorkerHealthTool) -> None:
    with patch("tools.prefect_worker_health_tool.make_prefect_client", return_value=None):
        result = tool.run(api_url="http://localhost:4200/api")
