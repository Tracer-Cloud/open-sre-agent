...
def test_check_version_freshness_skips_release_compare_for_local_dev(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "platform.release_version.development_install_doctor_version_detail",
        lambda c: f"{c} (editable install; skipped comparing to latest main build)",
    )
    monkeypatch.setattr("platform.release_version.fetch_latest_version", fetch_latest_version)
...
def test_check_version_freshness_up_to_date(monkeypatch) -> None:
    ...
    monkeypatch.setattr("platform.release_version.fetch_latest_version", fetch_latest_version)
    monkeypatch.setattr("platform.release_version.is_update_available", is_update_available)
...
def test_check_version_freshness_soft_fails_on_fetch_error(monkeypatch) -> None:
    ...
    monkeypatch.setattr("platform.release_version.fetch_latest_version", _raise)
