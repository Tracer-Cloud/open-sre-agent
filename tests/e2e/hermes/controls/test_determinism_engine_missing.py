...
def test_determinism_engine_missing() -> None:
    state = run_hermes_scenario("040-determinism-engine-missing")
    ...
    assert "determin" in response
    assert "same input" in response or "diverging" in response or "non-deterministic" in response
