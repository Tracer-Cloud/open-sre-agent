...
class TestDigestDeliveryReadiness:
    ...
    def test_rocketchat_ready_with_full_token_trio(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(
            "integrations.sentry.digest_delivery.resolve_mattermost_credentials",
            lambda _params: {},
        )
    ...
    def test_mattermost_ready_with_token_pair(self, monkeypatch: pytest.MonkeyPatch) -> None:
        # Isolate telegram/slack/rocketchat too: any_digest_delivery_ready()
        # must read True because of Mattermost specifically.
        monkeypatch.setattr(
            "integrations.sentry.digest_delivery.resolve_telegram_credentials",
            lambda _params: {},
        )
        ...
        monkeypatch.setattr(
            "integrations.sentry.digest_delivery.resolve_mattermost_credentials",
            lambda _params: {"server_url": "https://chat.example.com", "auth_token": "tok"},
        )
        assert mattermost_delivery_ready() is True
        assert delivery_provider_ready(Provider.MATTERMOST) is True
        assert any_digest_delivery_ready() is True

    def test_mattermost_not_ready_with_webhook_only(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """A webhook alone cannot target an explicit --chat-id destination."""
        monkeypatch.setattr(
            "integrations.sentry.digest_delivery.resolve_telegram_credentials",
            lambda _params: {},
        ...
        monkeypatch.setattr(
            "integrations.sentry.digest_delivery.resolve_mattermost_credentials",
            lambda _params: {"webhook_url": "https://chat.example.com/hooks/a"},
        )
        assert mattermost_delivery_ready() is False
        assert delivery_provider_ready(Provider.MATTERMOST) is False
        assert any_digest_delivery_ready() is False
    ...
    def test_none_ready(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert any_digest_delivery_ready() is False
        assert "Telegram, Slack, Rocket.Chat, or" in digest_delivery_setup_hint()
    ...
    def test_provider_specific_hint(self) -> None:
        ...
        assert "Mattermost" in digest_delivery_setup_hint(Provider.MATTERMOST)
