...
def _default_open_rollouts(monkeypatch: pytest.MonkeyPatch, codex_home: Path) -> None:
    ...
    monkeypatch.setattr(
        "app.tools.fleet_monitoring.token_sources.codex.open_files_for_pid", _all_rollouts
    )
...
def _patch_create_time(monkeypatch: pytest.MonkeyPatch, started_at_epoch: float) -> None:
    ...
    The source uses :func:`app.tools.fleet_monitoring.probe.started_at_for_pid` (the
