...
def _patch_cwd(monkeypatch: pytest.MonkeyPatch, cwd: Path) -> None:
    ...
    acceptance criterion that confines psutil to ``app/tools/fleet_monitoring/probe.py``);
    it calls :func:`app.tools.fleet_monitoring.probe.cwd_for_pid`. The test patches
