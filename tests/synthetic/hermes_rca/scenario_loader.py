...
# Registry of evidence sources: source_key -> (filename, validator function).
# Adding a new Hermes evidence type means declare the validator in
# hermes_schemas.py, then add one line below. The previous if-chain in
# _load_evidence grew with every new scenario type; this keeps the wiring
# in one place. Flagged by @VibhorGautam on #2183.
_EVIDENCE_LOADERS: dict[str, tuple[str, Callable[[dict[str, Any]], Any]]] = {
        "hermes_session_log": ("hermes_session_log.json", validate_hermes_session_log),
        "hermes_provider_traffic": (
                    "hermes_provider_traffic.json",
                    validate_hermes_provider_traffic,
        ),
        "hermes_config": ("hermes_config.json", validate_hermes_config),
        "hermes_runtime_state": ("hermes_runtime_state.json", validate_hermes_runtime_state),
        "hermes_message_history": (
                    "hermes_message_history.json",
                    validate_hermes_message_history,
        ),
        "hermes_kv_cache_state": ("hermes_kv_cache_state.json", validate_hermes_kv_cache_state),
        "hermes_cron_state": ("hermes_cron_state.json", validate_hermes_cron_state),
        "hermes_session_topology": (
                    "hermes_session_topology.json",
                    validate_hermes_session_topology,
        ),
        "hermes_orchestration_state": (
                    "hermes_orchestration_state.json",
                    validate_hermes_orchestration_state,
        ),
        "hermes_routing_decisions": (
                    "hermes_routing_decisions.json",
                    validate_hermes_routing_decisions,
        ),
        "hermes_memory_state": ("hermes_memory_state.json", validate_hermes_memory_state),
        "hermes_filesystem_state": (
                    "hermes_filesystem_state.json",
                    validate_hermes_filesystem_state,
        ),
}
...
def _load_evidence(scenario_dir: Path, available_evidence: list[str]) -> HermesScenarioEvidence:
        loaded: dict[str, Any] = dict.fromkeys(_EVIDENCE_LOADERS, None)
    for source_key, (filename, validator) in _EVIDENCE_LOADERS.items():
                if source_key in available_evidence:
                                loaded[source_key] = validator(_read_json(scenario_dir / filename))
                        return HermesScenarioEvidence(**loaded)
