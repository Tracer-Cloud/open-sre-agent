...
def test_already_up_to_date(
    ...
    monkeypatch.setattr("cli.interactive_shell.state.data_store.update.get_version", lambda: "1.2.3")
    monkeypatch.setattr(
        "cli.interactive_shell.state.data_store.update._fetch_latest_version", lambda: "1.2.3"
    )
...
def test_check_only_returns_1_when_update_available(
    ...
    monkeypatch.setattr("cli.interactive_shell.state.data_store.update.get_version", lambda: "1.0.0")
    monkeypatch.setattr(
        "cli.interactive_shell.state.data_store.update._fetch_latest_version", lambda: "1.2.3"
    )
    monkeypatch.setattr(
        "cli.interactive_shell.state.data_store.update._upgrade_via_install_script",
...
def test_update_install_script_success(
    ...
    monkeypatch.setattr(
        "cli.interactive_shell.state.data_store.update._upgrade_via_install_script", lambda _v: 0
    )
...
def test_update_install_script_failure_shows_retry_hint(
    ...
    monkeypatch.setattr(
        "cli.interactive_shell.state.data_store.update._upgrade_via_install_script", lambda _v: 1
    )
...
def test_fetch_error_returns_1(
    ...
    monkeypatch.setattr("cli.interactive_shell.state.data_store.update._fetch_latest_version", _raise)
...
def test_binary_install_upgrades_via_install_script(
    ...
    monkeypatch.setattr("cli.interactive_shell.state.data_store.update._is_binary_install", lambda: True)
    monkeypatch.setattr(
        "cli.interactive_shell.state.data_store.update._upgrade_via_install_script", lambda _v: 0
    )
...
def test_editable_install_prints_warning(
    ...
    monkeypatch.setattr("cli.interactive_shell.state.data_store.update._is_binary_install", lambda: False)
    monkeypatch.setattr(
        "cli.interactive_shell.state.data_store.update._is_editable_install", lambda: True
    )
    monkeypatch.setattr(
        "cli.interactive_shell.state.data_store.update._upgrade_via_install_script", lambda _v: 0
    )
...
def test_install_script_failure_windows_shows_powershell_hint(
    ...
    monkeypatch.setattr("cli.interactive_shell.state.data_store.update._is_windows", lambda: True)
    monkeypatch.setattr(
        "cli.interactive_shell.state.data_store.update._upgrade_via_install_script", lambda _v: 1
    )
...
def test_install_script_failure_unix_shows_curl_hint(
    ...
    monkeypatch.setattr("cli.interactive_shell.state.data_store.update._is_windows", lambda: False)
    monkeypatch.setattr(
        "cli.interactive_shell.state.data_store.update._upgrade_via_install_script", lambda _v: 1
    )
...
def test_upgrade_via_install_script_passes_version(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("cli.interactive_shell.state.data_store.update.subprocess.run", fake_run)
    monkeypatch.setattr("cli.interactive_shell.state.data_store.update._is_windows", lambda: False)
...
def test_development_install_doctor_detail_none_for_release_like_install(
    ...
    monkeypatch.setattr(
        "cli.interactive_shell.state.data_store.update._is_editable_install", lambda: False
    )
