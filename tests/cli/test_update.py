...
def test_already_up_to_date(
    ...
    monkeypatch.setattr(
        "cli.lifecycle.update._fetch_latest_version", lambda: "1.2.3"
    )
...
def test_update_install_script_success(
    ...
    monkeypatch.setattr(
        "cli.lifecycle.update._upgrade_via_install_script", lambda _v: 0
    )
...
def test_update_install_script_failure_shows_retry_hint(
    ...
    monkeypatch.setattr(
        "cli.lifecycle.update._upgrade_via_install_script", lambda _v: 1
    )
...
def test_editable_install_prints_warning(
    ...
    monkeypatch.setattr(
        "cli.lifecycle.update._is_editable_install", lambda: True
    )
    monkeypatch.setattr(
        "cli.lifecycle.update._upgrade_via_install_script", lambda _v: 0
    )
...
def test_development_install_doctor_detail_none_for_release_like_install(
    ...
    monkeypatch.setattr(
        "cli.lifecycle.update._is_editable_install", lambda: False
    )
