...
def test_check_version_freshness_skips_release_compare_for_local_dev(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "cli.interactive_shell.state.data_store.update.development_install_doctor_version_detail",
        lambda c: f"{c} (editable install; skipped comparing to latest release)",
    )
    monkeypatch.setattr(
        "cli.interactive_shell.state.data_store.update._fetch_latest_version", fetch_latest_version
    )
...
def test_check_version_freshness_up_to_date(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "cli.interactive_shell.state.data_store.update._is_update_available", is_update_available
    )
