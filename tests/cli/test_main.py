...
from surfaces.cli.app import _repl_preference, _run_without_subcommand, cli, main
...
class TestReplPreference:
    """Whether an invocation forces the interactive shell on, off, or defers.

    ``None`` is the subtle case: it means "no opinion", so ReplConfig still
    honors OPENSRE_INTERACTIVE and config.yml. Returning False there instead
    would stop a bare ``opensre`` from opening the shell.
    """

    def test_bare_invocation_defers_to_env_and_config(self) -> None:
        # Arrange: no --interactive flag typed, no --resume.

        # Act
        preference = _repl_preference(
            resume_session_id=None, interactive=True, passed_on_command_line=False
        )

        # Assert
        assert preference is None

    def test_click_default_does_not_count_as_an_opinion(self) -> None:
        """The flag defaults to True; only a typed flag is a decision.

        Reading the default as a decision is the bug this guards: it made every
        run pass cli_enabled=True and ignore the user's config.
        """
        # Act / Assert
        assert (
            _repl_preference(resume_session_id=None, interactive=True, passed_on_command_line=False)
            is None
        )

    def test_explicit_interactive_forces_the_shell_on(self) -> None:
        # Act / Assert
        assert (
            _repl_preference(resume_session_id=None, interactive=True, passed_on_command_line=True)
            is True
        )

    def test_explicit_no_interactive_forces_the_shell_off(self) -> None:
        # Act / Assert
        assert (
            _repl_preference(resume_session_id=None, interactive=False, passed_on_command_line=True)
            is False
        )

    def test_resume_forces_the_shell_on(self) -> None:
        """Resuming a session is meaningless without the shell."""
        # Act / Assert
        assert (
            _repl_preference(
                resume_session_id="abc123", interactive=True, passed_on_command_line=False
            )
            is True
        )

    def test_resume_outranks_an_explicit_no_interactive(self) -> None:
        """``--resume X --no-interactive`` still opens the session."""
        # Act / Assert
        assert (
            _repl_preference(
                resume_session_id="abc123", interactive=False, passed_on_command_line=True
            )
            is True
        )


class TestRunWithoutSubcommand:
    """A bare ``opensre``: open the shell, or fall back to the landing page."""

    @staticmethod
    def _tty(monkeypatch, *, value: bool) -> None:
        monkeypatch.setattr("surfaces.cli.app.sys.stdin.isatty", lambda: value)
        monkeypatch.setattr("surfaces.cli.app.sys.stdout.isatty", lambda: value)

    @staticmethod
    def _landing_spy(monkeypatch) -> list[object]:
        seen: list[object] = []
        monkeypatch.setattr("surfaces.cli.app.render_landing", lambda group: seen.append(group))
        return seen

    def test_without_a_terminal_it_never_opens_the_shell(self, monkeypatch) -> None:
        """Piped or redirected runs must not wait on a prompt nobody can answer."""
        # Arrange
        self._tty(monkeypatch, value=False)
        landing = self._landing_spy(monkeypatch)

        def _fail(**_kw: object) -> int:
            raise AssertionError("the shell must not open without a terminal")

        # Act
        with patch("surfaces.interactive_shell.run_repl", side_effect=_fail):
            exit_code = _run_without_subcommand(
                cli,
                resume_session_id=None,
                interactive=True,
                passed_on_command_line=False,
                layout=None,
                theme=None,
            )
    ...
    def test_resume_opens_a_session_even_when_config_disables_the_shell(self, monkeypatch) -> None:
        """``--resume`` is an explicit request; config must not veto it."""
        # Arrange
        self._tty(monkeypatch, value=True)
        landing = self._landing_spy(monkeypatch)
        monkeypatch.setattr(
            "config.repl_config.ReplConfig.load",
            classmethod(lambda _cls, **_kw: ReplConfig(enabled=False, layout="classic")),
        ...
        def _run(**kwargs: object) -> int:
            resumed.append(kwargs.get("resume_session_id"))  # type: ignore[arg-type]
            return 7

        # Act
        with patch("surfaces.interactive_shell.run_repl", side_effect=_run):
            exit_code = _run_without_subcommand(
                cli,
        ...
        # Assert: the shell ran and its exit code is returned, not swallowed.
        assert exit_code == 7
        assert resumed == ["abc123"]
        assert landing == []

    def test_layout_and_theme_reach_the_config_loader(self, monkeypatch) -> None:
        """The CLI overrides must not be dropped on the way to the shell."""
        # Arrange
        self._tty(monkeypatch, value=True)
        self._landing_spy(monkeypatch)
        seen: list[dict] = []

        @classmethod  # type: ignore[misc]
        def _spy(_cls, **kwargs):  # type: ignore[no-untyped-def]
            seen.append(kwargs)
            return ReplConfig(enabled=True, layout="classic")

        monkeypatch.setattr("config.repl_config.ReplConfig.load", _spy)

        # Act
        with patch("surfaces.interactive_shell.run_repl", return_value=0):
        ...
        # Assert
        assert seen[-1]["cli_layout"] == "pinned"
        assert seen[-1]["cli_theme"] == "green"
