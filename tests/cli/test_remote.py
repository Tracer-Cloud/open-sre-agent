...
def test_remote_health_requires_saved_or_explicit_url() -> None:
    ...
    with patch("surfaces.cli.wizard.store.load_remote_url", return_value=None):
...
def test_remote_health_uses_saved_url_and_persists_normalized_url() -> None:
    ...
    with (
        ...
        patch("surfaces.cli.wizard.store.load_remote_url", return_value="10.0.0.1"),
        patch(
            "infra.deployment.remote.client.RemoteAgentClient", return_value=client
        ) as mock_client_cls,
        patch("surfaces.cli.wizard.store.save_remote_url") as mock_save_remote_url,
    ):
        result = runner.invoke(cli, ["remote", "health"])
...
def test_remote_health_renders_probe_health_output() -> None:
    ...
    with (
        ...
        patch("surfaces.cli.wizard.store.save_remote_url"),
    ):
        result = runner.invoke(cli, ["remote", "health"])
...
def test_remote_trigger_persists_url_after_successful_run() -> None:
    ...
    with (
        patch("surfaces.cli.wizard.store.load_remote_url", return_value="10.0.0.1"),
        patch("infra.deployment.remote.client.RemoteAgentClient", return_value=client),
        patch("surfaces.cli.ui.renderer.StreamRenderer", return_value=renderer),
        patch("surfaces.cli.wizard.store.save_remote_url") as mock_save_remote_url,
    ):
...
def test_remote_ops_status_uses_provider_and_persists_scope() -> None:
    ...
    with (
        patch(
            "surfaces.cli.wizard.store.load_remote_ops_config",
        ...
        patch("surfaces.cli.wizard.store.save_remote_ops_config") as mock_save_scope,
    ):
        result = runner.invoke(
            cli,
            [
...
def test_remote_ops_status_prints_json() -> None:
    ...
    with (
        ...
        patch("surfaces.cli.wizard.store.save_remote_ops_config"),
    ):
