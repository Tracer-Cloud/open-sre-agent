...
from cli.interactive_shell.state.data_store.uninstall import _remove_path, run_uninstall
...
def test_run_uninstall_cancelled_by_user(
    ...
    monkeypatch.setattr("cli.interactive_shell.state.data_store.uninstall._data_dirs", lambda: [])
    monkeypatch.setattr(
        "cli.interactive_shell.state.data_store.uninstall._is_binary_install", lambda: False
    )
...
def test_run_uninstall_skips_missing_dirs(
    ...
    monkeypatch.setattr("cli.interactive_shell.state.data_store.uninstall._data_dirs", lambda: [missing])
    monkeypatch.setattr(
        "cli.interactive_shell.state.data_store.uninstall._is_binary_install", lambda: False
    )
    monkeypatch.setattr("cli.interactive_shell.state.data_store.uninstall._pip_uninstall", lambda: 0)
...
def test_run_uninstall_removes_existing_dir(
    ...
    d.mkdir()
    monkeypatch.setattr("cli.interactive_shell.state.data_store.uninstall._data_dirs", lambda: [d])
    monkeypatch.setattr(
        "cli.interactive_shell.state.data_store.uninstall._is_binary_install", lambda: False
    )
    monkeypatch.setattr("cli.interactive_shell.state.data_store.uninstall._pip_uninstall", lambda: 0)
...
def test_run_uninstall_pip_failure_shows_hint(
    ...
    monkeypatch.setattr("cli.interactive_shell.state.data_store.uninstall._pip_uninstall", lambda: 1)
    monkeypatch.setattr("cli.interactive_shell.state.data_store.uninstall._is_windows", lambda: False)
...
def test_run_uninstall_pip_failure_windows_hint(
    ...
    monkeypatch.setattr("cli.interactive_shell.state.data_store.uninstall._pip_uninstall", lambda: 1)
    monkeypatch.setattr("cli.interactive_shell.state.data_store.uninstall._is_windows", lambda: True)
...
def test_run_uninstall_binary_removes_executable(
    ...
    monkeypatch.setattr("cli.interactive_shell.state.data_store.uninstall._data_dirs", lambda: [])
    monkeypatch.setattr(
        "cli.interactive_shell.state.data_store.uninstall._is_binary_install", lambda: True
    )
    monkeypatch.setattr("cli.interactive_shell.state.data_store.uninstall.sys.executable", str(fake_exe))
...
def test_uninstall_command_yes_flag_skips_prompt() -> None:
    ...
    with (
        patch("cli.interactive_shell.state.data_store.uninstall._data_dirs", return_value=[]),
        patch("cli.interactive_shell.state.data_store.uninstall._is_binary_install", return_value=False),
        patch("cli.interactive_shell.state.data_store.uninstall._pip_uninstall", return_value=0),
    ):
...
def test_data_dirs_includes_legacy_config_opensre_path() -> None:
    from cli.interactive_shell.state.data_store.uninstall import _data_dirs
