...
from platform.safety.guardrails.audit import AuditLogger
from platform.safety.guardrails.engine import (
...
class TestEdgeCases:
    ...
    def test_real_world_api_key_and_aws_access_key_overlap(self) -> None:
        ...
        ``_STARTER_CONFIG`` of ``platform/safety/guardrails/cli.py``. The
...
class TestSingleton:
    def test_get_returns_engine(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        ...
        monkeypatch.setattr(
            "platform.safety.guardrails.engine.get_default_rules_path", lambda: config
        )
        reset_guardrail_engine()
    ...
    def test_reset_clears_singleton(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        monkeypatch.setattr(
            "platform.safety.guardrails.engine.get_default_rules_path",
