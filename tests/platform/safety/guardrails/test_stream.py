"""Tests for platform/safety/guardrails/stream.py.
...
from platform.safety.guardrails.engine import GuardrailEngine
from platform.safety.guardrails.rules import GuardrailAction, GuardrailRule
from platform.safety.guardrails.stream import GuardrailStream
...
def _rule(
    ...
    """Mirror tests/platform/safety/guardrails/test_engine.py's helper. Kept local so the
...
def stub_capture(monkeypatch: pytest.MonkeyPatch) -> list[dict[str, Any]]:
    ...
    monkeypatch.setattr("platform.safety.guardrails.stream.capture_agent_secret_detected", _stub)
    return calls
