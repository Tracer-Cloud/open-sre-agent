...
from platform.safety.auth.jwt_auth import (
    AsyncJWKSCache,
...
async def test_get_jwks_fetches_once_and_uses_cache_within_ttl() -> None:
    ...
    with (
        patch("platform.safety.auth.jwt_auth.time.time", side_effect=[1000.0, 1001.0]),
        patch("platform.safety.auth.jwt_auth.httpx.AsyncClient") as mock_async_client_cls,
    ):
