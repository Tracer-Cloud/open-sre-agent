...
def test_run_interactive_action_queues_setup_command(monkeypatch: Any) -> None:
    ...
    monkeypatch.setattr("interactive_shell.ui.components.choice_menu.repl_tty_interactive", lambda: True)
    session = ReplSession()
    console, buf = _capture()
...
def test_run_interactive_action_falls_back_to_guidance_without_tty(monkeypatch: Any) -> None:
    ...
    monkeypatch.setattr("interactive_shell.ui.components.choice_menu.repl_tty_interactive", lambda: False)
    session = ReplSession()
    console, buf = _capture()
    answer_cli_agent("can you configure sentry?", session, console)
