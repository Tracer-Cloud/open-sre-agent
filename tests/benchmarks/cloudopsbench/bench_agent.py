...
_TRIMMED_BENCH_SYSTEM_PROMPT = (
    ...
    "  Root-cause-shape consistency: when this rule fires and you have "
    "anchored on the alert's named service, the ROOT CAUSE you assign to "
    "that service must also be performance-shaped — pod_network_delay, "
    "pod_cpu_overload, pod_memory_pressure, pod_disk_io_saturation, or a "
    "similar resource / latency root cause. Do NOT assign a Runtime-shape "
    "root cause to a service you anchored on a performance alert: "
    "readiness_probe_*, liveness_probe_*, oom_killed, image_pull_*, "
    "deployment_zero_replicas, and similar configuration / crash root "
    "causes are Runtime_Fault root causes — they describe a different "
    "failure mode than the alert is reporting. If the evidence at the "
    "anchored service genuinely shows a probe / crash / config problem "
    "(not just slow response or saturation), then the alert was "
    "mis-classified upstream — but verify that with explicit pod-state "
    "evidence (restart count, exit code, probe failures in the events "
    "log) before overriding the alert's performance framing.\n"
