...
def _build_system_prompt() -> str:
    ...
    return (
        ...
        "Database-localization rule (CRITICAL — databases are first-class fault\n"
        "objects; symptoms on the upstream caller are NOT the fault location):\n"
        "  - When the investigation evidence or alert names a database service\n"
        "    (`tsdb-mysql`, `redis-cart`) AND the symptoms are DB-shaped (MySQL\n"
        "    `connection refused`, port mismatch, credential failure, Redis\n"
        "    connection error, pool/connection exhaustion message), rank-1\n"
        "    `fault_object` MUST be the database service itself —\n"
        "    `app/tsdb-mysql` or `app/redis-cart` — NOT the upstream service\n"
        "    that calls it. The upstream service appearing in the alert is a\n"
        "    downstream effect (it sees the DB's failure), not the fault origin.\n"
        "  - Common wrong-localization pattern to avoid: alert names\n"
        "    `ts-inside-payment-service` (or any other DB-backed caller),\n"
        "    investigation shows MySQL connection failure → do NOT localize\n"
        "    onto the caller; localize onto `app/tsdb-mysql`.\n"
        "  - Distinguishing MySQL root_causes (avoid the generic-bucket trap):\n"
        "      * `mysql_invalid_port`: pod spec / env / deployment yaml has a\n"
        "        MySQL port that doesn't match the DB's actual listener port.\n"
        "        Evidence cue: 'connection refused' on a port that is NOT 3306\n"
        "        (or whatever the DB actually serves), or explicit port mismatch\n"
        "        in the configuration vs the DB.\n"
        "      * `mysql_invalid_credentials`: authentication failure — 'access\n"
        "        denied for user', 'Authentication failed', wrong password\n"
        "        secret bound, missing password field.\n"
        "      * `db_connection_exhaustion`: actual pool saturation evidence —\n"
        "        connection count metrics at limit, 'too many connections' / 'max\n"
        "        connections exceeded' errors. Do NOT use as a catch-all for any\n"
        "        DB connection failure; reserve for genuine exhaustion evidence.\n"
        "  - When in doubt between `mysql_invalid_port` and\n"
        "    `db_connection_exhaustion`, prefer `mysql_invalid_port` —\n"
        "    misconfigured port is the more common Cloud-OpsBench fault shape\n"
        "    and the generic exhaustion bucket overfires.\n"
        "  - Distinguishing Redis root_causes (same generic-bucket trap applies):\n"
        "      * `missing_secret_binding`: the Redis password Secret is missing,\n"
        "        not bound to the pod, or the wrong key is referenced.\n"
        "        Evidence cue: 'NOAUTH Authentication required', 'WRONGPASS\n"
        "        invalid username-password pair', or no `requirepass` value\n"
        "        bound at all when the client sends AUTH.\n"
        "      * `db_readonly_mode`: Redis is in read-only mode — typically\n"
        "        after a failed failover, replica promotion, or explicit\n"
        "        `replica-read-only yes`. Evidence cue: 'READONLY You can't\n"
        "        write against a read only replica'.\n"
        "      * `db_connection_exhaustion`: explicit Redis maxclients\n"
        "        saturation — 'max number of clients reached' error or\n"
        "        connection-count metrics at the configured limit. Do NOT\n"
        "        default to this for generic Redis connection failures;\n"
        "        reserve for explicit maxclients evidence.\n"
        "  - General principle for any database fault (MySQL, Redis, or\n"
        "    others): pick the root_cause whose evidence cues match the\n"
        "    specific failure surface. `db_connection_exhaustion` is the\n"
        "    over-fired generic bucket — only use it when the evidence\n"
        "    explicitly shows pool/maxclients saturation, not whenever a DB\n"
        "    connection fails.\n\n"
