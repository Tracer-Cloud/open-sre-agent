...
dev: check-docker
	@echo "==> Setting up HealOps.ai dev environment..."
	uv sync --frozen --extra dev
	@if [ ! -f .env ]; then cp .env.example .env && echo "==> Created .env from .env.example (edit it to set API keys)"; fi
	docker compose -f docker-compose.dev.yml up -d
	@echo "==> Dev services started:"
	@echo "      Postgres: localhost:5432 (user=opensre, pass=opensre, db=opensre)"
	@echo "      Redis:    localhost:6379"
	@echo ""
	@echo "==> Next: set ANTHROPIC_API_KEY in .env, then run: make onboard"
...
# Run VaultCredentialProvider integration tests against LocalStack.
# Requires: docker run --rm -p 4566:4566 localstack/localstack
test-vault:
	@echo "Running VaultCredentialProvider integration tests against LocalStack..."
	AWS_ACCESS_KEY_ID=test AWS_SECRET_ACCESS_KEY=test AWS_DEFAULT_REGION=us-east-1 \
	AWS_ENDPOINT_URL=http://localhost:4566 \
	$(PYTHON) -m pytest -v -m vault_integration tests/test_vault_integration.py
