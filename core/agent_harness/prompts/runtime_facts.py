...
_GUIDANCE = (
    ...
    "never `ls` or subprocess. To check whether a host/port is reachable, use "
    "the sandbox with `socket.create_connection((host, port), timeout=3)` and "
    "`allow_network: true` — never `curl`, `ping`, or `nslookup`. Do NOT "
