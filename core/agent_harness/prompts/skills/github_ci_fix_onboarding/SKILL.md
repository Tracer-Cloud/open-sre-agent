...
fixed set of actions, emit a structured choice that the interactive shell can
render as a dropdown / selector. Never ask the user to type free-form text for
these decisions, and never end the turn with prose such as "please commit,
...
2. Immediately present this exact choice (do not rephrase the options):
...
3. Wait for the user's selection before doing anything else.
