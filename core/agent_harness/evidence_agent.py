        ...
        try:
            ...
            if error_reporter is not None:
                error_reporter.report(
                    exc,
                    context="core.agent_harness.evidence_agent.client",
                    expected=True,
                )
            return None
        self._llm = llm
...
def gather_tool_evidence(
    ...
    *,
    ...
    try:
        # Tool discovery + integration resolution happen here (has_usable_tools ->
        # build_tools/resolved_integrations); keep them inside the try so a raise
        # from tool-registry import or store/credential resolution is swallowed
        # rather than breaking the conversational turn.
