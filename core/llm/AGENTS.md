...
| `integrations/llm_providers/catalog.py` | Onboarding metadata (`SUPPORTED_PROVIDERS`) and model choices. |
| `integrations/llm_providers/env_sync.py` | `.env` synchronization when provider/model choices change. |
...
3. Add `ProviderSpec` in `config/llm_auth/provider_catalog.py` and matching `ProviderOption` in
   `integrations/llm_providers/catalog.py` (model env vars, defaults, `endpoint_env` if needed).
...
4. Update `integrations/llm_providers/env_sync.py` if you introduce new non-secret env keys; keep endpoint
