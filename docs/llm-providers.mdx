...
# Optional cost-tracking labels (JSON object; unset by default):
export VERTEX_AI_LABELS='{"team":"sre","env":"prod"}'
```
...
`VERTEX_AI_LABELS` attaches GCP billing/cost-tracking labels to each Vertex AI
request. Unset by default — no labels are sent unless configured. Label
values are not validated locally; malformed labels surface as a Vertex API
error.
