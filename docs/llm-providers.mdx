...
OpenSRE caches LLM clients on first use. To switch providers within a single process (tests, benchmarks), call `reset_llm_singletons()` from `services.llm_client` after updating the env vars; otherwise a fresh process picks up the new `LLM_PROVIDER` automatically.
...
- Runtime routing: [`services/llm_client.py`](https://github.com/Tracer-Cloud/opensre/blob/main/services/llm_client.py) (`_create_llm_client`).
- API-backed provider guide: [`services/AGENTS.md`](https://github.com/Tracer-Cloud/opensre/blob/main/services/AGENTS.md).
