...
| TrustedRouter| `trustedrouter`| `TRUSTEDROUTER_API_KEY`         | `trustedrouter/auto`                          | `trustedrouter/auto`                            |
...
with fast-changing or account-gated catalogs (OpenAI, OpenRouter, TrustedRouter, Gemini, NVIDIA,
Bedrock, local CLIs, Ollama, and DeepSeek) also accept custom model IDs:
...
`gpt-5.4-mini` is rejected. Ids under `trustedrouter/` are routing policies rather than
single models: each picks an upstream per request and fails over if it is down.
...
The full catalog is at [api.trustedrouter.com/v1/models](https://api.trustedrouter.com/v1/models).
