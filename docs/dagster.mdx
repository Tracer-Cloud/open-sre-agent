...
Fetches recent tick history for a Dagster schedule. Same `InstigationTick` shape as sensors. Identifying a schedule requires all three `ScheduleSelector` coordinates (repository location, repository, schedule name); each is exposed as a tool input.
