...
Real output from a run against this exact local instance -- captured from the tools' own raw results, not the agent's prose summary (which paraphrased the key count during testing):

```
Loading integrations: redis
↳ Redis · get redis list depth       -> depth=5, contents [a,b,c,d,e]
↳ Redis · get redis server info      -> 0 ops/sec, 0 rejected conns, 0 evictions
↳ Redis · get redis client list      -> 6 monitoring clients, 0 blocked
↳ Redis · get redis slowlog          -> 21 entries, including "DEBUG SLEEP 0.3" (300ms)
↳ Redis · get redis replication      -> role=master, 0 replicas
↳ Redis · get redis latency doctor   -> latency monitor disabled; no events
↳ Redis · scan redis keys            -> 2 keys: work_queue (list), session:1 (string, ttl 3579s)
...
and zero producer activity observed. The one slow entry (DEBUG SLEEP)
is a synthetic probe, not application traffic.
