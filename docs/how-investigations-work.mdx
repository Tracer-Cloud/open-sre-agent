...
**Config:** `LLM_PROVIDER` plus a matching API key from `opensre onboard`;
integrations via `opensre integrations setup`. See [Environment variables](/configuration/environment-variables).
