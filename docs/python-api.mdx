...
if not result.answered or result.cancelled or not result.primary_response_text:
...
if result.answered and not result.cancelled:
    print(result.primary_response_text)
...
Always check `result.answered` (and verify `not result.cancelled`) before
trusting chat text: when a turn fails or is cancelled (for example the LLM
provider is unreachable or times out), the failure details land in
