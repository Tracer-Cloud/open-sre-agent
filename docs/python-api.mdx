...
succeeded = (result.answered or result.final_intent == "cli_agent_handled") and not result.cancelled
...
if (result.answered or result.final_intent == "cli_agent_handled") and not result.cancelled:
    print(result.primary_response_text)
...
- For action-only turns (tools handled the request directly without an LLM call), the turn completes with `result.final_intent == "cli_agent_handled"`.
