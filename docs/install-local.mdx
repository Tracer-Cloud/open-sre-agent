...
The curl installer also soft-installs the [GitHub CLI](https://cli.github.com/) (`gh`) when it is missing (via Homebrew or apt). OpenSRE's `github_cli` chat tools need `gh` on `PATH`. Skip that step with:
