...
<Note>
The report compares the current window against the previous comparable window per metric. When a metric returns no data for the window it is reported explicitly as zero-ingestion, not silently widened or fabricated.
</Note>
