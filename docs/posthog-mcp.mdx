...
This is distinct from the [PostHog bounce-rate REST client](/posthog), which queries session bounce rates in your PostHog project. Use MCP when you want the agent to explore PostHog data directly during investigations. Configure bounce-rate monitoring with `opensre integrations setup posthog`.
