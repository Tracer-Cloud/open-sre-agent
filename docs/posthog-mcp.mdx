...
This is distinct from the [PostHog REST integration](/posthog), which stores project credentials for REST API access. Use MCP when you want the agent to explore PostHog data directly during investigations. Configure REST credentials with `opensre integrations setup posthog`.
