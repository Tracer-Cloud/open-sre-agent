...
Investigations started from a Slack thread land in the same table, and polling
by id works for those too as long as they belong to your organization. What does
not cross over is execution: each worker claims only records of its own origin,
so a run enqueued here is never picked up by the chat gateway and vice versa.
