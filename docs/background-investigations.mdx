...
This page is about the terminal. To start an investigation from a chat thread
instead, see [Slack](/messaging/slack#start-an-investigation-from-a-thread) —
those run detached by default and need no `/background on`.
