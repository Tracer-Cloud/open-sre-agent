...
<Warning>
The verifier currently does not redact secrets from this error: the `for url:` portion of the message includes the bot token (Telegram's API endpoint embeds the token in the path). Redact the token before sharing this error in bug reports or chat. See [#1481](https://github.com/Tracer-Cloud/opensre/issues/1481) and the linked issue for the verifier-side redaction follow-up.
</Warning>
