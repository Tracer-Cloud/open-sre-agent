...
No public HTTPS URL required. When `TELEGRAM_BOT_TOKEN` is set, **`opensre` auto-starts the gateway in poll mode** alongside the interactive shell (disable with `TELEGRAM_GATEWAY_AUTO_START=false`).
...
The gateway listens on `TELEGRAM_GATEWAY_HOST` (default `127.0.0.1`) and `TELEGRAM_WEBHOOK_PORT` (default `8443`). Put TLS termination on your reverse proxy and forward to that port.

External or mutating tool calls (for example `telegram_send_message`) prompt for **Approve / Deny** inline buttons in Telegram before running.
