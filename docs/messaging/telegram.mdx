...
- **Action tools** — `shell_run` and `investigation_start`, approval-gated over Telegram before execution

External or mutating tool calls (for example `telegram_send_message`, shell commands, and investigations) prompt for **Approve / Deny** inline buttons in Telegram before running.
