...
- **Action tools** — `shell_run` and `investigation_start` (auto-approved by default during alpha; set `TELEGRAM_GATEWAY_GATE_SIDE_EFFECTS=true` to require inline Approve/Deny)

Mutating or external tool calls can optionally prompt for **Approve / Deny** inline buttons when `TELEGRAM_GATEWAY_GATE_SIDE_EFFECTS=true`.
