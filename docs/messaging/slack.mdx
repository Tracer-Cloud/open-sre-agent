...
use: **the remote host cannot read your local machine's integration store /
keychain**. Deploy validates that secrets are present as **plaintext env vars**
in `.env`:

```bash
# Telegram-only, Slack-only, or both are accepted
TELEGRAM_BOT_TOKEN=…                 # optional if Slack is fully set
SLACK_BOT_TOKEN=xoxb-…
SLACK_APP_TOKEN=xapp-…
SLACK_ALLOWED_USERS=U0123ABCD
...
`make deploy` / `make deploy-gateway` require **at least one** of Telegram or
Slack Socket Mode. Partial Slack config (bot token without app token) aborts.
Missing allowlists are warnings (same shape as Telegram). Deploy health waits
for the `[gateway] ready` log line (works for Telegram-only, Slack-only, or both).

To give the deployed agent access to your observability integrations (so it
can pull Grafana, Datadog, or other data when asked in Slack), put those
credentials in `.env` too and list the variable names in
`OPENSRE_DEPLOY_EXTRA_ENV_KEYS`:

```bash
GRAFANA_INSTANCE_URL=…
GRAFANA_READ_TOKEN=…
OPENSRE_DEPLOY_EXTRA_ENV_KEYS=GRAFANA_INSTANCE_URL,GRAFANA_READ_TOKEN
```

The deploy ships every listed variable to both containers; the matching agent
tools become available automatically.

Fargate/terraform under `infra/slack/terraform/` injects Slack secrets via SSM —
see that README for the cloud path.
