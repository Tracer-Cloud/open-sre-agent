...
An allowlist is **required** for the Socket Mode bot, managed the same way as Telegram's:

```bash
opensre messaging allow -p slack -u U0123ABCD   # persisted in the integration store
```

or via the `SLACK_ALLOWED_USERS` env var (comma-separated member IDs; the integration store takes precedence when both are set). For dogfood only, you may set `SLACK_ALLOW_OPEN_WORKSPACE=1` instead (any workspace member can talk to the bot).
