...
### Deploying the Slack gateway to AWS

**All Slack deployment is Terraform** — a per-team Fargate stack, managed
separately from this repo. It runs the web API and the Slack gateway as ECS
services on a shared cluster, injects `SLACK_BOT_TOKEN` / `SLACK_APP_TOKEN`
from SSM SecureString at task start, and mounts persistent agent memory
(S3 Files) at `/workspace/memories/`.

The EC2 paths (`make deploy` / `make deploy-gateway`) are **Telegram-only**
and ignore `SLACK_*` variables with a validation warning. This is deliberate:
Socket Mode is single-consumer, so an EC2 gateway holding Slack tokens would
compete with the Fargate gateway and split events between them.
