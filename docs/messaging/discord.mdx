...
<Warning>
On Discord the investigation runs inside the turn, and a real one takes longer
than a turn is allowed to last — so you get a timeout instead of a report. Only
[Slack](/messaging/slack#start-an-investigation-from-a-thread) can hand the run
off and deliver the report back to the thread when it finishes. Use `/investigate`
here for short, cheap alerts only, or run it from the terminal.
</Warning>
