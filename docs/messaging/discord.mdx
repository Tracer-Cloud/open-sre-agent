...
- Slash command (registered during setup / finalize when application id + bot token are present):
...
| **`/investigate` missing** | Re-run `opensre integrations setup discord` to re-register commands. Global commands can take up to an hour to propagate. |
