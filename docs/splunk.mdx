...
label -- Splunk rejects an unrecognized name with `User "..." does not exist.` `audience`
is also required (a short free-text description of the token's purpose); omitting it
fails with `The following required arguments are missing: audience.`
</Warning>
...
until docker logs splunk-dev 2>&1 | grep -q "Ansible playbook complete"; do
  i=$((i + 1))
  [ "$i" -ge 60 ] && { echo "ERROR: Splunk never became ready" >&2; exit 1; }
  sleep 5
done
```

<Warning>
`splunk/splunk` publishes **amd64 only** -- on Apple Silicon, add `--platform
linux/amd64` (runs under emulation; startup takes several minutes longer than on
native amd64). Without `SPLUNK_GENERAL_TERMS=--accept-sgt-current-at-splunk-com`
alongside `SPLUNK_START_ARGS=--accept-license`, the container exits immediately
with `License not accepted` -- newer images require both, not just the license flag
...
never read or written and the `SPLUNK_*` vars above are the only source of connection info:

```bash
export OPENSRE_INTEGRATIONS_STORE_PATH=$(mktemp /tmp/opensre-splunk-demo.XXXXXX)
echo '{"version": 2, "integrations": []}' > "$OPENSRE_INTEGRATIONS_STORE_PATH"
```

Trigger a real investigation against the seeded event -- this is the actual supported
...
Real output from a run against this exact local instance (edited for length). An empty
store makes `opensre investigate` fall through to env-var resolution for *every*
integration, not just Splunk -- if your shell or machine already resolves another
integration from its own env vars, it rides along too (harmless, and unrelated to the
Splunk tool calls below):
...
The single registered tool was exercised, correctly scoping the blast radius to a
missing dependency rather than treating the absence of billing-api logs as inconclusive.
...
unset OPENSRE_INTEGRATIONS_STORE_PATH SPLUNK_URL SPLUNK_TOKEN SPLUNK_VERIFY_SSL
```
