...
`SPLUNK_*` vars above are the only source of connection info:
...
Trigger a real investigation against the seeded event:
...
integration resolved from your own env/keyring can ride along harmlessly, since the
store has no records to block fallback for any service:
...
unset OPENSRE_DEMO_STORE_DIR OPENSRE_INTEGRATIONS_STORE_PATH SPLUNK_URL SPLUNK_TOKEN SPLUNK_VERIFY_SSL
```
