...
- idempotency marker for create operations
...
- **update**: fetches the issue, adds the Slack follow-up as a comment, and patches title/labels/assignees only when those fields are explicitly present. It never replaces the issue body.
- **close**: fetches the issue, adds a closing comment, then patches `state=closed` and `state_reason=completed`. It never replaces the issue body.
