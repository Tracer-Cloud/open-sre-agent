...
Every `kubernetes_*` tool takes an optional `cluster` argument naming a registered instance. The agent discovers the names with `kubernetes_list_clusters`, then passes `cluster="gke-prod"` to any tool to target that cluster. Pass this whenever the request names or implies a specific cluster or environment; omitting it targets the default cluster only, which may not be the one asked about — on the workload-existence tools, an empty or not-found result from an unscoped call carries a pointer to `kubernetes_search_fleet` rather than reading as proof the workload is absent fleet-wide (see [Kubernetes](kubernetes) for exactly which tools). Unlike the Grafana alert-hint path, selection happens per tool call during the investigation or chat turn, so one investigation can span several clusters.
