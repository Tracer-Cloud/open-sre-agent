...
memory plus a generated `MEMORY.md` index. OpenSRE creates that folder (and an
empty index) the first time memory is used in a chat or via `/memory`. They are
stored **locally and unencrypted**. Every chat and action turn includes the
stored facts (summaries plus bodies, within a size budget) in prompts sent to
your configured LLM provider. You can open, edit, or delete the files directly
at any time; the index rebuilds on the next write.
