...
```text
/remote-sync status
/remote-sync setup --provider aws --bucket my-bucket
/remote-sync sync --pull-only
```

Sync conversation history and memory to a user-owned object store. See [Remote sync](/configuration/remote-sync) for details.
