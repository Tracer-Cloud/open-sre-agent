...
TEMPO_USERNAME=<tenant-id>         # optional (basic auth)
TEMPO_PASSWORD=<token>             # optional (basic auth)
TEMPO_ORG_ID=<tenant>              # optional (X-Scope-OrgID for multi-tenant)
```
...
| `TEMPO_ORG_ID` | _(empty)_ | Tenant ID sent as `X-Scope-OrgID` |
