...
- **One agent per logical session; one turn at a time on it.** The agent
  enforces this: an overlapping `handle` or `dispatch` raises `AgentBusyError`
  (from `core.agent_harness.runtime`) rather than interleaving. Queue *before*
  `handle` — the gateway holds a per-session lock across it — and do not catch
  the error to retry.
