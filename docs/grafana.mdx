...
OpenSRE verifies Grafana TLS certificates by default. For an internal certificate
authority, provide its PEM bundle during onboarding or configure:

```bash
GRAFANA_VERIFY_SSL=true
GRAFANA_CA_BUNDLE=/etc/ssl/certs/internal-grafana-ca.pem
```

For a trusted local development instance only, you can disable verification with
`GRAFANA_VERIFY_SSL=false`. Prefer a CA bundle in production because disabling
verification permits man-in-the-middle attacks.
