...
The tool also accepts `target_group_arns`, the singular `target_group_arn` alias, and `load_balancer_arn` directly as tool parameters — at least one of `target_group_arns`/`target_group_arn` or `load_balancer_arn` must be provided (via the resolved sources or the tool call) for the tool to run.
