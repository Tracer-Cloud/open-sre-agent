...
<Steps>
  ...
  <Step title="Onboard">
    Run the onboarding wizard to configure the LLM provider OpenSRE uses to reason:

    ```bash
    opensre onboard
    ```

    After you pick a provider and a model, the wizard sends one tiny request to that provider with the credential you just entered — so a typo, an expired key, or an Ollama model you have not pulled yet is caught here, not on your first investigation. Add integrations later with `opensre integrations setup <service>` when you want OpenSRE to query a specific tool.
