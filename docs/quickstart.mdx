...
<Steps>
  ...
  <Step title="Setup">
    ...
    After GitHub authorization, you pick a provider and a model. The wizard sends one tiny request to that provider with the credential you just entered — so a typo, an expired key, or an Ollama model you have not pulled yet is caught here, not on your first investigation. When setup finishes, OpenSRE opens the interactive shell. Add integrations later with `opensre integrations setup <service>` when you want OpenSRE to query a specific tool.
