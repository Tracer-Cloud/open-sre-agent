...
never read or written and the `HELM_*` vars above are the only source of connection info:

```bash
export OPENSRE_INTEGRATIONS_STORE_PATH=$(mktemp /tmp/opensre-helm-demo.XXXXXX)
echo '{"version": 2, "integrations": []}' > "$OPENSRE_INTEGRATIONS_STORE_PATH"
```

Trigger a real investigation against the deployed release -- this is the actual supported
entrypoint, not an internal import:

```bash
opensre investigate --input-json '{
  "alert_name": "HelmReleaseErrors",
  "severity": "warning",
  "labels": {
  ...
  "annotations": {
    "summary": "Errors observed after helm upgrade of demo release"
  }
}'
```
...
integration, not just Helm -- if your shell or machine already resolves another
integration from its own env vars, it rides along too (harmless, and unrelated to the
Helm tool calls below):

```
Loading integrations: helm, telegram
↳ Helm · helm list releases

Triage complete: One Helm release found in the demo namespace -- demo
(chart nginx-25.1.1, revision 1) with status deployed. No failed/pending/
superseded releases detected across the cluster.

root_cause: The HelmReleaseErrors alert fired during the incident window,
but the sole Helm release in the demo namespace is currently deployed at
revision 1, updated inside the incident window. No failed, pending, or
unknown Helm releases exist cluster-wide, consistent with a transient
reconciliation error that has since self-resolved.
```
...
unset OPENSRE_INTEGRATIONS_STORE_PATH OSRE_HELM_INTEGRATION HELM_KUBE_CONTEXT HELM_NAMESPACE
```
