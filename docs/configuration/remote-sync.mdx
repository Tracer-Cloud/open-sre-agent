...
**Warning:** `rotate-passphrase` and `reencrypt` are not safe to run concurrently — not at the same time as each other, and not from two machines at once. 
Doing so can corrupt the manifest or leave some data undecryptable. Make sure no other rotation or reencrypt is running before starting one.
...
**Out of scope:** Anyone who can **write** to the store, can corrupt it. Encryption stops them reading your history; it does not stop them wrecking the manifest — the small key record OpenSRE keeps in the store, which holds ecrypted root keys.

If it gets corrupted, sync refuses in both directions, leaving your local files alone as it as - even though your passphrase is intact. 
