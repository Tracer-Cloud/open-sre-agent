---
title: "Sync to your own S3 bucket"
description: "Carry your conversations and memory between machines, using a bucket you own."
---

opensre keeps everything on your laptop by default. Turn on remote sync and it
also mirrors your conversation history and memory to an S3 bucket **you** own,
so a second machine picks up where the first left off.

Your AWS credentials are read from the usual places — environment, profile, SSO
session. opensre never stores them.
...
| `OPENSRE_REMOTE_SYNC_BUCKET` | Bucket you own. Required |
| `OPENSRE_REMOTE_SYNC_PREFIX` | Key prefix, default `opensre`. Machines that should share history must use the same one |
| `OPENSRE_REMOTE_SYNC_REGION` | Region override; otherwise your AWS config decides |
| `OPENSRE_REMOTE_SYNC_PROFILE` | Named AWS profile, if opensre should not use your default |
...
<Note>
Every machine that should share your history needs the **same bucket and the
same prefix**. Change the prefix only when you want a separate, isolated set of
...
```bash
opensre sync run
opensre sync run --pull-only    # new machine, fetch only
opensre sync run --push-only    # back up without pulling
```
...
<Warning>
Keep the bucket private. It holds your incident conversations and everything
...
bucket or from your other machines. Delete it in the bucket too if you want it
gone everywhere.
