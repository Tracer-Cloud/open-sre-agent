...
**Out of scope:** an attacker who has write access to the store *and* a passphrase you have since rotated away from. Write access covers the key record OpenSRE keeps in the store, so an attacker can put back a copy from before you rotated. A machine that syncs against the restored copy then seals new uploads under the retired key. OpenSRE does not detect this — it keeps no local record of which key generation it last saw, so a machine syncing for the first time has nothing to compare against either.

<Warning>
A leaked passphrase needs `rotate-passphrase` and `reencrypt`. Only `rotate-passphrase` re-wraps the same content keys, so whoever kept a copy of the old key record and knows the old passphrase can still read every object — including ones uploaded after the rotation. `reencrypt` starts a new key generation and re-seals everything under it.

Even that is not post-compromise recovery: it cannot take write access away from whoever already had it. Revoke the store credentials that leaked alongside the passphrase, and if you cannot be sure they are revoked, sync to a **new bucket or prefix** and treat the old one as exposed.
</Warning>
