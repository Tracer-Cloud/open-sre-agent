...
`setup` asks, and prompts for the passphrase twice if you say yes:
...
For scripted setups, `--encrypt` (or `--no-encrypt`) answers that question up
front and skips the prompt — supply the passphrase in
`OPENSRE_REMOTE_SYNC_PASSPHRASE`, since there is no terminal to type it into.
