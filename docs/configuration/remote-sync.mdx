...
Later setup runs preserve the stored encryption mode when neither encryption
flag is given. Use `--no-encrypt` only when you intentionally want plaintext,
and choose a new empty prefix because plaintext sync refuses a prefix already
initialized for encryption.

The first encrypted upload adds a small encrypted verification object to the
prefix. Later syncs validate it before uploading anything, so a missing or
incorrect passphrase fails before remote content can be changed. An empty
`--pull-only` run remains read-only. A nonempty prefix without the marker is
rejected instead of being treated as encrypted.
Likewise, a plaintext sync refuses a prefix that already has the marker, so
turning encryption off cannot silently overwrite encrypted content.

Pulling an object also rejects plaintext, tampered ciphertext, and ciphertext
moved to another path. There is no recovery without the original passphrase.
