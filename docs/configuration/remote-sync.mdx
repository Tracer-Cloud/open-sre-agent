...
All surfaces share one service (`platform/filestorage/service.py`):
...
| Gateway (Slack / Telegram / Discord) | same `/remote-sync` slash command (headless ports) |

Roots follow the active principal scope — laptop `~/.opensre` unbound, or the
org user's `sessions/` + `memory/` tree when a gateway principal is bound.
