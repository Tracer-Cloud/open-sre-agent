...
`SLACK_WEBHOOK_URL`, `ROCKETCHAT_WEBHOOK_URL`, and `MATTERMOST_WEBHOOK_URL` stay
store / plain env only.
...
| `MATTERMOST_SERVER_URL` | | Mattermost server base URL | `integrations/_catalog_impl` |
| `MATTERMOST_AUTH_TOKEN` | | Mattermost personal access token | `integrations/_catalog_impl` |
| `MATTERMOST_DEFAULT_CHANNEL` | | Default Mattermost channel id | `integrations/_catalog_impl` |
| `MATTERMOST_WEBHOOK_URL` | | Mattermost incoming webhook URL | `integrations/_catalog_impl` |
