...
| `GRAFANA_VERIFY_SSL` | `true` | Verify the Grafana TLS certificate; set `false` only for trusted development instances | `integrations/_catalog_impl` |
| `GRAFANA_CA_BUNDLE` | | Path to a custom CA bundle for Grafana TLS verification | `integrations/_catalog_impl` |
