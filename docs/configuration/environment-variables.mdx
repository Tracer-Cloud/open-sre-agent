...
|----------|---------|-------------|--------|
| `LLM_PROVIDER` | `anthropic` | Primary LLM provider (`anthropic`, `openai`, `openrouter`, `deepseek`, `gemini`, `nvidia`, `minimax`, `groq`, `bedrock`, `ollama`) | `config` |
| `LLM_MAX_TOKENS` | Provider-specific | Maximum tokens for LLM responses | `config` |
...
See [multi-instance-integrations](/integrations/multi-instance-integrations) for detailed format.
...
- See [LLM Providers](/integrations/llm-providers) for detailed provider setup instructions
