...
|----------|---------|-------------|--------|
| `LLM_PROVIDER` | `anthropic` | Primary LLM provider (`anthropic`, `openai`, `openrouter`, `trustedrouter`, `deepseek`, `gemini`, `nvidia`, `minimax`, `groq`, `azure-openai`, `bedrock`, `ollama`, `codex`, `claude-code`, `cursor`, `gemini-cli`, `antigravity-cli`, `opencode`, `kimi`, `copilot`, `grok-cli`, `pi`) | `config` |
...
| Variable | Default | Description | Module |
|----------|---------|-------------|--------|
| `TRUSTEDROUTER_API_KEY` | | API key for TrustedRouter | `llm_credentials` |
| `TRUSTEDROUTER_MODEL` | | Shared model for all tasks | `config` |
| `TRUSTEDROUTER_REASONING_MODEL` | | Override model for reasoning | `config` |
| `TRUSTEDROUTER_TOOLCALL_MODEL` | | Override model for tool calling | `config` |
