...
| `BEDROCK_AWS_PROFILE` |  | Named AWS profile for Bedrock only, isolated from `AWS_PROFILE` used by infrastructure investigation |
| `BEDROCK_AWS_ROLE_ARN` |  | AWS role to assume for Bedrock only (cross-account setup); takes precedence over `BEDROCK_AWS_PROFILE` |
| `BEDROCK_AWS_EXTERNAL_ID` |  | External ID for the `BEDROCK_AWS_ROLE_ARN` assume-role call |
| `BEDROCK_AWS_REGION` |  | Region override for Bedrock only, when it differs from `AWS_REGION` |
