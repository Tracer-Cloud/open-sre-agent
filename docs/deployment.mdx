...
Runs waiting for a scheduler worker appear as `pending` in run history.
Pending runs older than 24 hours are marked failed when the scheduler starts,
so interrupted queued work does not remain pending indefinitely.
...
limits determines their effective concurrency.
