...
- Replication lag: If a massive write-heavy workload on the primary generates WAL faster than the read replica can replay it, resulting in ReplicaLag spikes, the root cause is `resource_exhaustion` driven by the write workload on the primary. Watch out for red herrings: if concurrent analytics queries cause high CPU, do NOT classify the root cause as CPU-driven if the actual failing metric (like ReplicaLag) is driven by the write-heavy workload. Treat the CPU spike as an unrelated confounder/red herring and explicitly describe it as a red herring (i.e., not the root cause) in the final answer. Do not treat it as a second root cause or a contributing cause. Explicitly state that the analytics SELECT is not the root cause when the evidence shows it is causally independent from WAL generation. If the `ReplicaLag` metric is missing, infer lag from RDS events (e.g., 'exceeded 900s') and high `TransactionLogsGeneration`. This rule applies to single-fault replication scenarios. It should only be bypassed if storage exhaustion or another independent fault is explicitly corroborated by direct evidence (e.g. FreeStorageSpace depletion, write-failure signals, or named write workload evidence).
- Compositional Faults: If two completely independent workloads cause two separate faults simultaneously (e.g., CPU saturation from an analytics SELECT AND storage exhaustion from an audit_log INSERT), explicitly identify BOTH as independent root causes (do not merge them into a single IOPS fault). You MUST explicitly state they are two independent, coincidental faults. Provide evidence for both the CPU-driving workload and the storage-driving workload, using the exact mechanism named in the evidence (e.g. audit_log, pg_audit, WAL growth, table bloat, temp-file growth). When storage exhaustion is driven by a named write/logging workload, use the exact mechanism named in the evidence (for example `audit_log`, `pg_audit`, WAL growth, table bloat, or temp-file growth) instead of generic terms like logs, transaction logs, temporary tables, or ingestion. Use `resource_exhaustion` as ROOT_CAUSE_CATEGORY and describe both causes clearly in ROOT_CAUSE (e.g., "Two independent root causes: ..."). Trace each causal chain separately in CAUSAL_CHAIN. You MUST explicitly state that connection growth is a symptom of blocked writers, not connection exhaustion. If replication lag is explicitly present in the evidence (e.g. ReplicaLag metric, RDS replication event, or TransactionLogsGeneration data), explicitly state that ReplicaLag is a downstream symptom of the write/WAL burst and not an independent root cause. If those signals are not present in the evidence, do not mention ReplicaLag. NEVER diagnose `connection_exhaustion` as a root cause when connections spike due to a blocked write queue.
