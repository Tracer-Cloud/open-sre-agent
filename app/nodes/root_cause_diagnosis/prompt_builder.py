...
ROOT_CAUSE_CATEGORY:
<exactly one of: configuration_error | code_defect | data_quality | dependency_failure | infrastructure | healthy | unknown | resource_exhaustion | connection_exhaustion | storage_exhaustion | cpu_saturation | replication_lag | checkpoint_io_storm | dual_resource_exhaustion | application_tier_load_spike>
...
- Connection pool leaks (exhausting `max_connections`) must be categorized as `connection_exhaustion`. High CPU is often just a secondary symptom of accumulated idle sessions. If connections are near 100%, the root cause is connection exhaustion, not CPU saturation. If connections are near 100% AND CPU is near 100%, look for a single shared root cause: a connection pool leak holding open scan-heavy queries, causing both (do not treat them as two independent problems).
- Storage exhaustion (when `FreeStorageSpace` approaches 0) blocks all writes and causes Write IOPS to collapse to 0. The root cause category must be `storage_exhaustion`. If the `FreeStorageSpace` metric is completely missing, you MUST infer storage exhaustion from indirect signals: WriteIOPS dropping to 0, WriteLatency spiking, and RDS events indicating 'ran out of storage space'.
- A single bad query driving CPU near 100% while connections and storage are healthy must be categorized as `cpu_saturation` (e.g. missing index, full table scans at high ReadIOPS). Pay close attention to Performance Insights to identify the exact query.
