...
Carries the same shape as ``core/domain/state/agent_state.py``'s ``evidence`` records so
...
class BusMessage:
    ...
    ``"<name>:<pid>"`` convention used by ``app.tools.fleet_monitoring.conflicts.WriteEvent``.
