...
def mongodb_atlas_config_from_env() -> MongoDBAtlasConfig | None:
    ...
    public_key = get_opt("MONGODB_ATLAS_PUBLIC_KEY").strip()
    private_key = get_opt("MONGODB_ATLAS_PRIVATE_KEY").strip()
    project_id = get_opt("MONGODB_ATLAS_PROJECT_ID").strip()
    ...
    return build_mongodb_atlas_config(
        {
            ...
            "base_url": get_opt("MONGODB_ATLAS_BASE_URL", DEFAULT_ATLAS_BASE_URL).strip(),
        }
    )
