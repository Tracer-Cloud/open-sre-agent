"""Shared derivation and channel-syntax helpers for the RCA report formatters.

This module owns the pure helpers that every channel-specific renderer
(Slack mrkdwn, Slack Block Kit, Telegram HTML, Discord markdown) and the
:mod:`sections` builder share. Moving them out of ``report.py`` breaks the
module-level import cycle CodeQL flagged on PR #2057: ``report.py`` is now
a thin re-export shim that no other module depends on transitively.

Three concern groups, in order of appearance below:

1. **Channel-syntax conversion** — :func:`_sanitize_for_slack`,
   :func:`_to_telegram_html_body`, :func:`_mrkdwn_section`. These translate
   LLM-emitted Markdown into channel-native syntax (or wrap text in Slack
   Block Kit section blocks).
2. **Evidence / provenance / correlation extraction** —
   :func:`_format_provenance_lines`, :func:`_format_correlation_lines`,
   :func:`_resolve_evidence_tags`, :func:`_get_top_error_log`. These read
   :class:`ReportContext` and return structured bullets / strings ready
   for rendering.
3. **Root-cause derivation** — :func:`_derive_root_cause_sentence` and its
   private helpers. Picks the best one-sentence summary from the
   investigation's raw root-cause text and validated claims.

The CloudWatch-link helpers :func:`render_cloudwatch_link` and
:func:`render_cloudwatch_link_html` are channel-specific renderers but
live here too because they're called from multiple per-channel modules
and read ctx directly; folding them into :class:`Section`'s ``extras``
contract is a follow-up.

All helpers are intentionally underscore-prefixed in the public API
(except the two ``render_cloudwatch_link*`` functions kept for backwards
compatibility with their original names) — they are module-internal
implementation details consumed by ``sections.py`` and ``renderers/``.
"""
...
# ---------------------------------------------------------------------------
# Channel-syntax conversion
# ---------------------------------------------------------------------------


def _sanitize_for_slack(text: str) -> str:
    """Convert standard Markdown formatting into Slack mrkdwn.

    Slack does not render ``#`` headers, ``**bold**``, or other CommonMark
    constructs. This converts the common patterns the LLM emits so the
    rendered Slack message uses native mrkdwn (``*bold*`` and ``*headings*``).
    """
...
def _to_telegram_html_body(text: str) -> str:
    """Convert mixed Slack-style text (headers, ``*bold*``, ``\\`code\\```, ``<url|label>``) to Telegram HTML."""  # noqa: E501
...
def _mrkdwn_section(text: str) -> dict[str, Any] | None:
    ...
    Slack section blocks have a 3000 char limit per text field — we
    truncate at 2990 to leave room for the trailing ellipsis. Returns
    ``None`` when text is empty so callers can skip empty results.
    """
...
# ---------------------------------------------------------------------------
# Evidence, provenance, and correlation extraction
# ---------------------------------------------------------------------------


def _format_provenance_lines(ctx: ReportContext) -> list[str]:
    """Return ``• Label: summary`` bullets from ``ctx["source_provenance"]``."""
...
def _format_correlation_lines(ctx: ReportContext) -> tuple[list[str], list[str]]:
    """Return ``(signal_lines, driver_lines)`` from ``ctx["correlation"]``.

    Mirrors the format introduced in PR #1877: signals show source + score,
    drivers show confidence + rationale.
    """
...
# ---------------------------------------------------------------------------
# CloudWatch link rendering (read ctx directly; channel-specific output)
# ---------------------------------------------------------------------------


def render_cloudwatch_link(ctx: ReportContext) -> str:
    """Render CloudWatch logs link as Slack mrkdwn, or empty string."""
