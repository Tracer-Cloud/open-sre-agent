...
Selected when ``DATABASE_URL`` is set; requires the ``postgresql`` extra. Pooling
and connection bounds come from :class:`~gateway.core.storage.postgres.PostgresDatabase`.
"""
...
class PostgresInvestigationStore:
    ...
    def __init__(self, dsn: str, *, database: PostgresDatabase | None = None) -> None:
        self._db = (
            ...
            else PostgresDatabase(dsn, max_connections=_POOL_MAX_CONNECTIONS)
        )
        self._db.run_schema(_SCHEMA)
